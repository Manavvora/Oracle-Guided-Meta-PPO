...
def meta_ppo_test(num_components):
    ...
    components_df_repeated = pd.concat([components_df]*num_instances, ignore_index=True)
    # components_df_repeated = components_df_repeated.iloc[:num_components]
    component_ids = list(components_df_repeated['component_id'])
    component_ids = component_ids[:num_components]
    replacement_costs = list(components_df_repeated['replacement_cost'])
    replacement_costs = replacement_costs[:num_components]
    inspection_costs = list(components_df_repeated['inspection_cost'])
    inspection_costs = inspection_costs[:num_components]
    component_type_names = list(components_df_repeated['component_type_name'])
    component_type_names = component_type_names[:num_components]
    shape_factors = list(components_df_repeated['shape'])
    shape_factors = shape_factors[:num_components]
    scale_factors = list(components_df_repeated['scale'])
    scale_factors = scale_factors[:num_components]

    total_components = len(component_ids)
    print(f'Total components: {total_components}')

    # value_iteration_policies_path = os.path.join(value_iteration_dir, 'value_iteration_policies_repair')
    budget_vals = np.load(f'benchmark_results/budget_split_{total_components}_components.npy')
    # budget_vals = [500]
    num_runs = 100
    ...
    state_log = np.zeros((len(component_ids),num_runs,100,2))

    for i in tqdm(range(len(component_ids)), desc="Processing Components"):
        ...
        mdp_policy_path = f'benchmark_data/oracle_policies_optimal_budget_split/Component_{component_ids[i]}/value_iteration_policy_budget_{budget_vals[i]}.npy'
        ...
        for k in range(num_runs):
            ...
            while not terminated or not truncated:
                ...
                state_log[i,k,step] = env.true_state[:2]
    ...
    np.save(f'benchmark_results/oracle_guided_meta_ppo/ppo_avg_ttfs_{total_components}_components_optimal_split.npy', avg_ttfs)
    np.save(f'benchmark_results/oracle_guided_meta_ppo/ppo_replace_actions_data_{total_components}_components_optimal_split.npy', replace_actions_data)
    np.save(f'benchmark_results/oracle_guided_meta_ppo/ppo_inspect_actions_data_{total_components}_components_optimal_split.npy', inspect_actions_data)
    np.save(f'benchmark_results/oracle_guided_meta_ppo/ppo_cost_incurred_data_{total_components}_components_optimal_split.npy', cost_incurred_data)
    np.save(f'benchmark_results/oracle_guided_meta_ppo/ppo_state_log_{total_components}_components_optimal_split.npy', state_log)
