...
# budget_split_dir = os.path.join(parent_dir, 'pomdp_solver/results')
budget_split = np.load(f'results/baseline_budget_split.npy')
...
class ValueIteration:
    ...
    def value_iteration(self):
        ...
        while delta > self.theta:
            delta = 0
            iters += 1
            # print(f'Iteration = {iters-1}; Delta = {delta}')
            count = 0
                    ...
                    for a in range(self.env.num_actions):
                        for s_new in range(self.env.num_healths):
                            if a == 0:
                                ...
                                c_new = c + 1
                            # print(s, c, s_new, c_new, a)
                            # print(f'Reward = {self.env.reward_func([s,self.env.cost_array[c]])}')
...
def process_component(component_id, budget):
    ...
    Vfunc[:, 100] = -10
    if not os.path.exists(f'data/oracle_policies_baseline_budget_split/Component_{component_id}'):
        os.makedirs(f'data/oracle_policies_baseline_budget_split/Component_{component_id}')
    ...
    print(f'Running value iteration for component {component_id} with budget {budget}')
    V, pi = ValueIteration(env, Vfunc).value_iteration()
    Vfunc = V
    np.save(f'data/oracle_policies_baseline_budget_split/Component_{component_id}/value_iteration_policy_budget_{budget}.npy', pi)

components_df = pd.read_csv(os.path.join(env_dir, '1000_components_data.csv'))
